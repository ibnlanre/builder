...
The reason why `$use` is a property on the root node and not a method is to facilitate retrieving the type of nested keys within the register, without having to import the register object alongside the builder object. If the `$use` property had been a method, accessing nested types would require square brackets to access the nested keys, which would be cumbersome for [TypeScript][typescript] users.

```typescript copy
//  type Bar = ReturnType<typeof builder.$use>['foo']['bar'];
type Bar = typeof builder.$use.foo.bar; // number
```
