...
The root node, the starting point and first node of the [builder][3] object, behaves differently from other nodes when interacting with the `use` and `get` methods. When accessed using the `use` method, the root node returns the [register][4] itself, which is effectively the input to the [createBuilder][2] function.

This guide elucidates the behavior of the `use` and `get` methods specifically on the root node of the [builder][3] object.
...
Invoking the `use` method on the root node returns the [register][4] in its entirety, thereby enabling access to its values. This behavior is distinct from that of other nodes, which generate a key derived from the nested keys within the [register][4], rather than providing direct access to the nested values.
