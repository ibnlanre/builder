...
A `register` is a plain object that contains the keys and values that you want to access. The `register` can contain nested objects, functions, or values. To effectively use the [@ibnlanre/builder][1] package, a register is passed as an argument to the [createBuilder][2] function, which returns a [builder][3] object. The [builder][3] object adds a layer of abstraction to the `register` that facilitates the dynamic generation of keys. The `register` is the **soul** of the [builder][3] object, as it is the source of the keys and values contained within the [builder][3] object. This guide explains how to define a `register` and how to structure it effectively.
