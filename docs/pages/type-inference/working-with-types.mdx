...
To provide additional type information to a value in the [register][4], the `typeValue` function, exported by the [@ibnlanre/builder][1] package, can be used. Using this function is recommended over manual type assertions, as it ensures that the type information is correctly associated with a value. To expatiate, the type information would not be made accessible through the `use` and `get` methods as it is not an actual value in the [register][4]. Regardless, the type information can be accessed using the `unbuild` property.
